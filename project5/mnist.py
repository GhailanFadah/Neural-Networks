...
def get_mnist(N_val, path='data/mnist'):
    ...
    x_train_f = preprocess_mnist(x_train)
    x_test = np.load(path+"/x_test.npy")
    x_test_f = preprocess_mnist(x_test)
    y_test = np.load(path+"/y_test.npy")
    x_val_f = preprocess_mnist(x_val)
    ...
    return x_train_t, y_train_t, x_test_t, y_test_t, x_val_t, y_val_t
